...
export interface VirtualClockOptions {
  /**
   * How far `virtualStart` must precede `realStart` for the deployment to count
   * as anchored a year back. 360 rather than 365: the anchors are generated by
   * `date -u -d '1 year ago'`, and a leap year plus a few minutes of deploy skew
   * should not fail a correctly anchored stack.
   */
  ...
  private validate(raw: RawServerTime, readAt: Date): ServerTime {
    ...
    if (realStart && virtualStart) {
      ...
      if (gapDays < this.minAnchorGapDays) {
        problems.push(
          `virtualStart must precede realStart by at least ${this.minAnchorGapDays} days to cover a ` +
            `year (gap is ${gapDays.toFixed(1)} days: ${virtualStart.toISOString()} → ${realStart.toISOString()})`,
        );
      }
    }
