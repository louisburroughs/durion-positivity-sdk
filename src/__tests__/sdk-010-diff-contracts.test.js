 ...
 */
function writeFakeApiFile(baseDir, operationIds) {
...
describe('SDK-010 DiffContracts: argument validation', () => {
    it('when_missing_previous_argument_then_exits_1', () => {
        const result = (0, node_child_process_1.spawnSync)('bash', [SCRIPT_PATH, '--current', currDir], {
            encoding: 'utf-8',
        });
    ...
    it('when_missing_current_argument_then_exits_1', () => {
        const result = (0, node_child_process_1.spawnSync)('bash', [SCRIPT_PATH, '--previous', prevDir], {
            encoding: 'utf-8',
        });
        expect(result.status).toBe(1);
    });
...
describe('SDK-010 DiffContracts: ADDED operations are reported', () => {
    it('when_newOperation_in_current_only_then_stdout_contains_ADDED_newOperation', () => {
        const result = (0, node_child_process_1.spawnSync)('bash', [SCRIPT_PATH, '--previous', prevDir, '--current', currDir], { encoding: 'utf-8' });
...
describe('SDK-010 DiffContracts: graceful handling when --previous directory is absent', () => {
    it('when_previous_dir_does_not_exist_then_exits_0_and_all_current_ops_are_ADDED', () => {
        ...
        const result = (0, node_child_process_1.spawnSync)('bash', [SCRIPT_PATH, '--previous', nonexistentPrev, '--current', currDir], { encoding: 'utf-8' });
